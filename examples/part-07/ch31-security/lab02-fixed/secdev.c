/*
 * secdev.c - Lab 2 of Chapter 31 (fixed).
 *
 * Adds bounds checks to the read and write paths.  Every other bug
 * from Lab 1 is preserved for later labs to fix.
 */
...
secdev_modevent(module_t mod __unused, int event, void *arg __unused)
{
	...
	case MOD_LOAD:
		...
		printf("secdev: loaded (Lab 2 fixed)\n");
		return (0);
