/*
 * secdev.c - Lab 4 of Chapter 31 (fixed).
 *
 * Adds two layers of access control:
 *   - filesystem-level:    device-node mode 0600, root owner
 *   - in-kernel:           priv_check(td, PRIV_DRIVER) in d_open plus a
 *                          jailed(td->td_ucred) refusal
 */
...
secdev_modevent(module_t mod __unused, int event, void *arg __unused)
{
	...
	case MOD_LOAD:
		...
		printf("secdev: loaded (Lab 4 fixed, root only)\n");
		return (0);
