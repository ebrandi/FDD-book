/*
 * secdev.c - Lab 4 of Chapter 31 (starter file).
 *
 * Privilege-check focus.  The starter still creates the device node
 * world-writable and allows any caller to open it.  Your task is to
 * restrict access to privileged users and to close the jail escape
 * path.
 */
    ...
    int devtype __unused, struct thread *td __unused)
{
	...
	/*
	 * TODO(lab4): add a priv_check(td, PRIV_DRIVER) call here and
	 * return the error it produces if the caller lacks privilege.
	 *
	 * TODO(lab4): additionally refuse opens from jailed credentials
	 * with jailed(td->td_ucred).  Device nodes can be exposed inside
	 * a jail, so the in-kernel check is the last defence.
	 */
	return (0);
}
...
secdev_modevent(module_t mod __unused, int event, void *arg __unused)
{
	...
	case MOD_LOAD:
		...
		/*
		 * TODO(lab4): change the default device-node mode to 0600
		 * so that the filesystem layer alone blocks unprivileged
		 * access.  This is the outer layer of the defence.
		 */
		args.mda_mode = 0666;
		...
		printf("secdev: loaded (Lab 4 starter)\n");
		return (0);
