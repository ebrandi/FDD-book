/*
 * secdev.c - Lab 2 of Chapter 31 (starter file).
 *
 * This file is a copy of Lab 1 with "TODO(lab2)" markers added at the
 * two places where you need to add bounds checks.  Your task is to make
 * sc_buf-sized reads and writes safe regardless of what the caller asks
 * for.
 *
 * The buffer overflow and the unchecked read length are the only two
 * issues you are asked to fix in this lab.  Do not worry about the
 * information leak, the privilege check, the logging rate, or the
 * detach order here; they are covered in later labs.
 */
...
secdev_read(struct cdev *dev __unused, struct uio *uio, int ioflag __unused)
{
	...
	mtx_lock(&sc->sc_mtx);
	/*
	 * TODO(lab2): replace the unbounded read with a bounded one.  The
	 * caller may request more or fewer bytes than sc_buflen; copy out
	 * only min(sc_buflen - uio_offset, uio_resid) bytes.  If
	 * uio_offset is already past the valid data, return 0 bytes.
	 */
...
secdev_write(struct cdev *dev __unused, struct uio *uio, int ioflag __unused)
{
	...
	mtx_lock(&sc->sc_mtx);
	/*
	 * TODO(lab2): clamp the copy length to SECDEV_BUFSIZE.  A common
	 * pattern is:
	 *
	 *	size_t amount = MIN(uio->uio_resid, SECDEV_BUFSIZE);
	 *	error = uiomove(sc->sc_buf, amount, uio);
	 *
	 * Remember to update sc_buflen to reflect how many bytes were
	 * actually copied.
	 */
...
secdev_modevent(module_t mod __unused, int event, void *arg __unused)
{
	...
	case MOD_LOAD:
		...
		printf("secdev: loaded (Lab 2 starter)\n");
		return (0);
