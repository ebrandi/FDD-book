/*
 * secdev.c - Lab 5 of Chapter 31 (fixed).
 *
 * Adds a ppsratecheck-gated log line to the default branch of the
 * ioctl dispatcher.  A process that issues malformed ioctls in a
 * tight loop can no longer flood dmesg.
 */
...
secdev_modevent(module_t mod __unused, int event, void *arg __unused)
{
	...
	case MOD_LOAD:
		...
		printf("secdev: loaded (Lab 5 fixed)\n");
		return (0);
