/*
 * evdemo.c - Lab 3: adds kqueue() support to the evdemo driver.
 *
 * Changes from Lab 2:
 *   - Adds a filterops structure for EVFILT_READ.
 *   - Adds d_kqfilter callback that attaches the caller's knote to
 *     sc->sc_rsel.si_note.
 *   - Adds KNOTE_LOCKED(&sc->sc_rsel.si_note, 0) in the producer
 *     path, inside the softc mutex.
 *   - Adds knlist_clear() to the detach path before seldrain().
 *   - Reports EV_EOF to kqueue waiters during detach.
 */
...
evdemo_modevent(module_t mod, int event, void *arg)
{
	...
	case MOD_LOAD:
		...
		printf("evdemo: loaded (lab03-kqueue)\n");
		break;
