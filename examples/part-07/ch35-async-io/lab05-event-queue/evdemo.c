/*
 * evdemo.c - Lab 5: polished event queue with sysctl observability.
 *
 * Changes from Lab 4:
 *   - Adds a dev.evdemo.stats sysctl subtree with qlen, posted,
 *     consumed, dropped, selwakeups, knotes_delivered, sigio_sent.
 *   - Adds dev.evdemo.burst that posts a batch of events.
 *   - Leaves the three async mechanisms unchanged but exposes their
 *     counters.
 */
...
static int	evdemo_burst_sysctl(SYSCTL_HANDLER_ARGS);

static SYSCTL_NODE(_dev, OID_AUTO, evdemo, CTLFLAG_RW, 0,
    "evdemo driver");
static SYSCTL_NODE(_dev_evdemo, OID_AUTO, stats, CTLFLAG_RW, 0,
    "evdemo runtime statistics");
...
static int
evdemo_burst_sysctl(SYSCTL_HANDLER_ARGS)
{
	struct evdemo_softc *sc = evdemo_sc_global;
	struct evdemo_event ev;
	int count = 0;
	...
	for (i = 0; i < count; i++) {
		nanotime(&ev.ev_time);
		ev.ev_type = EVDEMO_EV_TYPE_BURST;
		ev.ev_code = (uint32_t)i;
		ev.ev_value = count;
		evdemo_post_event(sc, &ev);
	}
	return (0);
...
evdemo_modevent(module_t mod, int event, void *arg)
{
	...
	case MOD_LOAD:
		...
		printf("evdemo: loaded (lab05-event-queue)\n");
		break;
