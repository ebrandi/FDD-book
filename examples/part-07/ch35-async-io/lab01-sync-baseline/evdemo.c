/*
 * evdemo.c - Lab 1: synchronous baseline for the evdemo driver.
 *
 * This first version supports:
 *   - open(), close(), and blocking read() on /dev/evdemo.
 *   - An internal ring-buffer event queue.
 *   - A callout that posts a synthetic "tick" event once per second.
 *   - A sysctl (dev.evdemo.trigger) that posts a synthetic event on
 *     demand.
 *
 * No asynchronous I/O mechanisms yet: no poll(), no kqueue(), no
 * SIGIO. Reads block in a condition variable until an event is
 * available. Later labs layer the asynchronous mechanisms on top of
 * this baseline.
 */
...
evdemo_modevent(module_t mod, int event, void *arg)
{
	...
	case MOD_LOAD:
		...
		printf("evdemo: loaded (lab01-sync-baseline)\n");
		break;
