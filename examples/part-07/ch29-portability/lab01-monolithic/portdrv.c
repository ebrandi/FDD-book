/*
 * portdrv.c - Chapter 29 reference driver, Lab 1 starting point.
 *
 * A single-file driver that exercises a small, fictitious device
 * attached to a PCI bus. It compiles and loads, but it mixes core
 * logic, bus-specific code, and register access without any of the
 * portability structure the chapter will introduce in later labs.
 *
 * The point of keeping every concern in one file is not that this is
 * good code. It is that it is the baseline. Every lab after this one
 * improves on a specific axis of portability. Having the starting
 * point in front of you makes the improvements easier to see.
 */
...
portdrv_attach(device_t dev)
{
	...
	sc->sc_last_data = bus_read_4(sc->sc_bar, REG_DATA);
	device_printf(dev, "portdrv version %d attached\n", PORTDRV_VERSION);
...
portdrv_read(struct cdev *cdev, struct uio *uio, int flag)
{
	...
	uint32_t val = bus_read_4(sc->sc_bar, REG_DATA);
...
portdrv_write(struct cdev *cdev, struct uio *uio, int flag)
{
	...
	if (error != 0)
		return (error);
	bus_write_4(sc->sc_bar, REG_DATA, val);
	sc->sc_last_data = val;
	return (0);
}
