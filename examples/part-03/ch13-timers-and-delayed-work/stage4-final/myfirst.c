 ...
 *
 * Chapter 13, Stage 4: the final timer-enhanced driver.
 *
 * Builds on Chapter 12 Stage 4 by adding three callouts:
 ...
 * (heartbeat_interval_ms, watchdog_interval_ms, tick_source_interval_ms)
 * that enables, disables, and configures the firing interval.
 * The default for each interval is 0 (disabled), preserving the
 * Chapter 12 default behaviour for users who do not opt in.
 *
 * Locking strategy.
 *
 * sc->mtx protects:
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 13 Stage 4)");
