 ...
 *
 * Chapter 13, Stage 3: heartbeat + watchdog + tick source.
 *
 * Builds on Stage 2 by adding the tick source callout:
 *   - heartbeat_co: periodic logging of cbuf usage and byte counts.
 *   - watchdog_co: periodic detection of stalled buffer drainage.
 *   - tick_source_co: periodic injection of synthetic bytes.
 *
 * Each callout uses sc->mtx as its lock (via callout_init_mtx) and
 * is drained at detach via callout_drain. Each has a sysctl
 * (heartbeat_interval_ms, watchdog_interval_ms,
 * tick_source_interval_ms) that enables, disables, and configures
 * the firing interval. The default for each is 0 (disabled).
 *
 * Stage 4 is the integrated final driver with the standard macro
 * vocabulary and the seven-phase detach pattern documented in
 * comments.
 *
 * Locking strategy.
 *
...
#define	MYFIRST_VERSION		"0.7-tick-source"
...
struct myfirst_softc {
	...
	/* Chapter 13: callouts and their tunables. */
...
/*
 * Chapter 13: callout callbacks. Each runs with sc->mtx held (the
 * kernel acquires it via callout_init_mtx). Each must check
 * is_attached at the top and exit without re-arming if false.
 */
...
/* Sysctl handlers for the three timer intervals. */
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 13 Stage 3)");
...
myfirst_attach(device_t dev)
{
	...
	/* Chapter 13: callout interval sysctls. */
	SYSCTL_ADD_PROC(&sc->sysctl_ctx,
	    SYSCTL_CHILDREN(device_get_sysctl_tree(dev)),
