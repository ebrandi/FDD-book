 ...
 *
 * Chapter 13, Stage 2: heartbeat + watchdog callouts.
 *
 * Builds on Stage 1 by adding the watchdog callout:
 ...
 * (heartbeat_interval_ms, watchdog_interval_ms) that enables,
 * disables, and configures the firing interval. The default for
 * each interval is 0 (disabled).
 *
 * Stage 3 will add the tick source callout. Stage 4 is the
 * integrated final driver with the standard macro vocabulary and
 * the seven-phase detach pattern.
 *
 * Locking strategy.
 *
 * sc->mtx protects:
...
#define	MYFIRST_VERSION		"0.7-watchdog"
...
/* Sysctl handlers for the heartbeat and watchdog intervals. */
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 13 Stage 2)");
...
myfirst_detach(device_t dev)
{
	...
	/* Chapter 13 Stage 2: drain heartbeat + watchdog callouts. */
