 ...
 *
 * Chapter 13, Stage 1: heartbeat callout only.
 *
 * Builds on Chapter 12 Stage 4 by adding one callout:
 *   - heartbeat_co: periodic logging of cbuf usage and byte counts.
 *
 * The callout uses sc->mtx as its lock (via callout_init_mtx) and
 * is drained at detach via callout_drain. The sysctl
 * heartbeat_interval_ms enables, disables, and configures the firing
 * interval. The default is 0 (disabled), preserving the Chapter 12
 * default behaviour for users who do not opt in.
 *
 * Stage 2 will add the watchdog callout. Stage 3 will add the tick
 * source. Stage 4 is the integrated final driver with the standard
 * macro vocabulary and the seven-phase detach pattern.
 *
 * Locking strategy.
 *
 * sc->mtx protects:
...
#define	MYFIRST_VERSION		"0.7-heartbeat"
...
struct myfirst_softc {
	...
	/* Chapter 13 Stage 1: heartbeat callout. */
...
/* Sysctl handler for the heartbeat interval (Stage 1). */
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 13 Stage 1)");
...
myfirst_attach(device_t dev)
{
	...
	/* Chapter 13 Stage 1: heartbeat sysctl. */
	SYSCTL_ADD_PROC(&sc->sysctl_ctx,
	    SYSCTL_CHILDREN(device_get_sysctl_tree(dev)),
...
myfirst_detach(device_t dev)
{
	...
	/* Chapter 13 Stage 1: drain the heartbeat callout. */
