 ...
 *
 * Chapter 15, Stage 2: add the stats cache with sx upgrade pattern.
 *
 * Builds on Stage 1 by adding a read-mostly statistics cache:
 *   sc->stats_cache_sx                  sx lock protecting the cache
 *   sc->stats_cache_bytes_10s           cached bytes_written snapshot
 *   sc->stats_cache_last_refresh_ticks  cache freshness timestamp
 *   sc->stats_cache_refreshes           diagnostic counter
 *
 * Readers of dev.myfirst.N.stats.bytes_written_10s take the sx shared;
 * when the cache is stale they attempt sx_try_upgrade; on failure they
 * drop-and-reacquire with sx_xlock, re-check, refresh, downgrade, and
 * serve the value.
 *
 * Stage 3 refines EINTR/ERESTART handling; Stage 4 adds recovery
 * coordination and the myfirst_sync.h wrapper layer.
 *
 * Original Chapter 14 header preserved below:
 *
...
#include <sys/sema.h>           /* Chapter 15 Stage 1 */
#include <machine/atomic.h>     /* Chapter 15 Stage 1: writers_inflight */
...
#define	MYFIRST_VERSION		"0.9-coordination-stage2"
#define	MYFIRST_WRITERS_DEFAULT	4
#define	MYFIRST_BULK_MAX	64
#define	MYFIRST_CACHE_STALE_HZ	1   /* Stage 2: stale after 1 second. */
...
struct myfirst_softc {
	 ...
	 * writers_inflight is incremented before sema_wait and
	 * decremented after the matching sema_post on every exit path.
	 * Detach uses it to wait for all writers to leave sema code
	 * before calling sema_destroy, which closes the sema_destroy
	 * use-after-free race described in Section 8.
	 */
	struct sema		writers_sema;
	int			writers_limit;
	int			writers_trywait_failures;
	int			writers_inflight;   /* atomic int */
...
myfirst_sysctl_writers_limit(SYSCTL_HANDLER_ARGS)
{
	...
	/* Raising: post extra slots immediately.  Lowering: best-effort
	 * (new entries observe the lower cap as the counter drains). */
	for (i = 0; i < delta; i++)
		sema_post(&sc->writers_sema);
	return (0);
}

/* Chapter 15 Stage 1: read the current semaphore value. */
static int
...
/* Chapter 15 Stage 2: refresh the cached bytes_written statistic.
...
/* Chapter 15 Stage 2: read the cached value using the
 * upgrade-promote-downgrade pattern.  Readers share the sx; the
 * staleness check may promote briefly to refresh; a failed
 * try_upgrade drops and re-acquires exclusively, then re-checks. */
static int
...
myfirst_write(struct cdev *dev, struct uio *uio, int ioflag)
{
	...
	/* Chapter 15 Stage 1: mark this thread as potentially using
	 * the writers_sema.  Detach waits for writers_inflight to
	 * drop to zero before calling sema_destroy, so every path
	 * past this point and through the matching decrement is
	...
	/* Chapter 15 Stage 1: admission control. */
	if (ioflag & IO_NDELAY) {
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 15 Stage 2)");
...
myfirst_attach(device_t dev)
{
	...
	/* Chapter 15 Stage 1: writer-cap semaphore and its drain counter. */
	...
	/* Chapter 15 Stage 1: writer-cap sysctls. */
	SYSCTL_ADD_PROC(&sc->sysctl_ctx,
	    SYSCTL_CHILDREN(device_get_sysctl_tree(dev)),
	...
	SYSCTL_ADD_PROC(&sc->sysctl_ctx, SYSCTL_CHILDREN(sc->sysctl_tree),
	    ...
	    "Current writer semaphore counter value");
	SYSCTL_ADD_INT(&sc->sysctl_ctx, SYSCTL_CHILDREN(sc->sysctl_tree),
	...
	/* Chapter 15 Stage 2: stats cache sysctls. */
	SYSCTL_ADD_PROC(&sc->sysctl_ctx, SYSCTL_CHILDREN(sc->sysctl_tree),
	...
	SYSCTL_ADD_INT(&sc->sysctl_ctx, SYSCTL_CHILDREN(sc->sysctl_tree),
	    ...
	    &sc->stats_cache_refreshes, 0,
	    "Number of times the stats cache was refreshed");
...
myfirst_detach(device_t dev)
{
	...
	/* Chapter 15 Stage 1: wake any writer blocked in sema_wait so it
	 * can observe is_attached=0 and return ENXIO.  writers_limit
	 * slots are an upper bound on the number of simultaneously
	 * blocked waiters. */
	for (int i = 0; i < sc->writers_limit; i++)
		sema_post(&sc->writers_sema);

	/* Chapter 15 Stage 1: drain in-flight writers.  Every path
	 * that may be inside sema_wait, in the critical section, or
	 * between sema_wait return and the matching sema_post is
	 * counted by writers_inflight.  Waiting for the counter to
	 * reach zero ensures no thread is still using the sema's
	 * internal mutex or condition variable when we later call
	 * sema_destroy. */
	...
	/* Chapter 15 Stage 1: destroy the writer-cap semaphore. */
	sema_destroy(&sc->writers_sema);
