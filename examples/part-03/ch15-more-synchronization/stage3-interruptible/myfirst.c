 ...
 *
 * Chapter 15, Stage 3: refine EINTR/ERESTART handling.
 *
 * Builds on Stage 2 by:
 *   - introducing the MYFIRST_WAIT_PARTIAL sentinel to replace the
 *     magic -1 return value on the partial-progress path,
 *   - making the cv_wait return-value switch explicit
 *     (0 / EWOULDBLOCK / EINTR / ERESTART / default),
 *   - preserving partial progress on signal interruption.
 *
 * Stage 4 adds recovery coordination and the myfirst_sync.h wrapper
 * layer.
 *
    ...
    struct uio *uio)
{
	...
	while (cbuf_used(&sc->cb) == 0) {
		...
		/* Chapter 15 Stage 3: explicit handling of every
		 * cv_*_sig return value. */
		switch (error) {
		case 0:
			break;
		case EWOULDBLOCK:
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 15 Stage 3)");
