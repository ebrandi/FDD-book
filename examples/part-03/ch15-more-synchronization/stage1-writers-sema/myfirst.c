 ...
 *
 * Chapter 15, Stage 1: add the writer-cap semaphore.
 *
 * Builds on Chapter 14 Stage 4 by adding one primitive:
 *   sc->writers_sema              counts available writer slots
 *   sc->writers_limit             current configured cap
 *   sc->writers_trywait_failures  diagnostic counter
 *
 * Stage 2 adds the stats cache sx; Stage 3 refines EINTR/ERESTART
 * handling; Stage 4 adds recovery coordination and the
 * myfirst_sync.h wrapper layer.
 *
...
#define	MYFIRST_VERSION		"0.9-coordination-stage1"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 15 Stage 1)");
