 ...
 *
 * Chapter 12, Stage 3: sx-protected configuration subsystem.
 *
 * Builds on Stage 2 (bounded reads) by adding a runtime configuration
 * subsystem protected by sc->cfg_sx, an sx(9) lock. Three configurable
 * fields: debug_level, soft_byte_limit, nickname. Lock order: sc->mtx
 * before sc->cfg_sx (mutex first, sx second).
 *
 * Locking strategy.
 *
 * sc->mtx protects:
...
#define	MYFIRST_VERSION		"0.6-sx-config"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 12 Stage 3)");
