 ...
 *
 * Chapter 12, Stage 1: condition variables replace anonymous channels.
 *
 * Builds on Chapter 11 Stage 5 (counter9 + KASSERTs). Replaces the
 * shared anonymous wakeup channel (&sc->cb) with two named condition
 * variables: data_cv ("data is available") and room_cv ("room is
 * available"). Wait helpers use cv_wait_sig; signallers target the
 * appropriate cv only.
 *
 * Locking strategy.
 *
...
#define	MYFIRST_VERSION		"0.6-cv-channels"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 12 Stage 1)");
