 ...
 *
 * Chapter 14, Stage 2: the private taskqueue.
 *
 * Builds on Chapter 14 Stage 1 by creating a private taskqueue
 * (sc->tq) serviced by one worker thread at PWAIT.  Only selwake_task
 * lives here; Stage 3 adds coalescing demonstrations and Stage 4 adds
 * the delayed-reset timeout task.
 *
...
#define	MYFIRST_VERSION		"0.8-taskqueues-stage2"
...
struct myfirst_softc {
	...
	/* Chapter 14 Stage 2: private taskqueue and one task. */
	struct taskqueue       *tq;
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 14 Stage 2)");
...
myfirst_attach(device_t dev)
{
	...
	/* Chapter 14 Stage 2: initialise the task. */
