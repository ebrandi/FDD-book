 ...
 *
 * Chapter 14, Stage 3: deliberate coalescing and a delayed reset.
 *
 * Builds on Stage 2 by adding bulk_writer_task (to demonstrate
 * deliberate coalescing via the ta_pending rule) and
 * reset_delayed_task (a timeout_task that fires in thread context).
 * Stage 4 is this same driver with consolidated documentation and the
 * final version string.
 *
 * The driver owns a private taskqueue and the following tasks:
 *
...
#define	MYFIRST_VERSION		"0.8-taskqueues-stage3"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 14 Stage 3)");
