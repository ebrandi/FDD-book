 ...
 *
 * Chapter 11, Stage 3: migrate byte counters to counter(9).
 *
 * Builds on Stage 2 by replacing sc->bytes_read and sc->bytes_written,
 * which were plain uint64_t fields protected by sc->mtx, with per-CPU
 * counter_u64_t handles managed by the counter(9) API. The migration
 * removes those two fields from the set protected by sc->mtx and lets
 * the data path update them lock-free via counter_u64_add.
 *
 * Locking strategy.
 *
 * sc->mtx protects:
 ...
 *   - sc->is_attached
 *
 * sc->bytes_read, sc->bytes_written are counter(9) per-CPU counters and
 * protect themselves; the mutex is no longer required to update them.
 *
 * Locking discipline (unchanged from Stage 2 except for the counters):
...
#define	MYFIRST_VERSION		"0.5-concurrency"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 11 Stage 3)");
