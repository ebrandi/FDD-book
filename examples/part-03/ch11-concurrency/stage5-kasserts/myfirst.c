 ...
 *
 * Chapter 11, Stage 5: KASSERTs throughout, ready for a debug kernel.
 *
 * Builds on Chapter 11 Stage 3 (counter(9) migration) by adding KASSERT
 * calls to every cbuf helper, every wait helper, and every transition that
 * has an invariant worth checking. With INVARIANTS enabled in the kernel,
 * any logic bug that violates one of these invariants becomes an immediate,
 * loud panic with a backtrace, instead of silent corruption that might
 * surface much later.
 *
 * Adds MYFIRST_LOCK/UNLOCK/ASSERT
 * macros for future lock-class changes, plus an explicit version string.
 *
 * Adds d_poll, selinfo for read and write readiness, and refactors
 * the I/O handlers to go through a small set of mutex-asserting
 * helpers (myfirst_buf_read, myfirst_buf_write, myfirst_wait_data,
 * myfirst_wait_room).
 *
 * Locking strategy.
 *
 ...
 *   - sc->is_attached
 *
 * sc->bytes_read and sc->bytes_written are counter(9) per-CPU
 * counters; they protect themselves and do not need sc->mtx.
 *
 * Locking discipline:
...
#define	MYFIRST_VERSION		"0.5-kasserts"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 11 Stage 5)");
