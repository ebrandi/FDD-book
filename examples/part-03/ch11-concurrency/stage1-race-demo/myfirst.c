 ...
 *
 * Chapter 11, Stage 1: DELIBERATELY UNSAFE race-demo driver.
 *
 * This driver is structurally identical to the Chapter 10 Stage 4 driver,
 * except that compiling with -DRACE_DEMO (the default in this directory's
 * Makefile) makes MYFIRST_LOCK/UNLOCK/ASSERT expand to no-ops. The mutex
 * is initialized and destroyed correctly so that mtx_sleep continues to
 * work, but the I/O paths run with no mutual exclusion.
 *
 * The purpose of the driver is to demonstrate the data corruption that
 * arises in a circular buffer when multiple threads can read and write
 * concurrently without synchronization. See Section 2 of Chapter 11 for
 * the exercise that uses it.
 *
 * DO NOT load this module on a production machine. Expect inconsistent
 * data, possible kernel warnings, and (rarely) panics.
 */
...
#define	MYFIRST_VERSION		"0.5-race-demo"
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 11 Stage 1 (UNSAFE))");
