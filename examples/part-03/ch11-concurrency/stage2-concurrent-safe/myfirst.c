 ...
 *
 * Chapter 11, Stage 2: concurrent-safe with documented locking.
 *
 * Functionally identical to the Chapter 10 Stage 4 driver. The
 * MYFIRST_LOCK/UNLOCK/ASSERT macros add a single point of indirection
 * around the mutex so that future stages (sx locks, finer-grained
 * mutexes) can change the lock class without touching the call sites.
 * The version string is bumped to "0.5-concurrency" to reflect the
 * fact that the locking story is now explicit.
 *
 * Locking strategy.
 *
 * sc->mtx protects:
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 11 Stage 2)");
