 ...
 *
 * Chapter 10, Stage 3: blocking and non-blocking I/O.
 *
 * Builds on Stage 2 by adding mtx_sleep(9) on the empty/full
 * condition, IO_NDELAY interpretation as EAGAIN, and wakeup(9)
 * matching every state transition.  Detach now sets is_attached=0
 * and wakes any sleepers before tearing the mutex down.
 *
 * Stage 4 layers d_poll/selrecord/selwakeup on top of this.
 */
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 10 Stage 3)");
