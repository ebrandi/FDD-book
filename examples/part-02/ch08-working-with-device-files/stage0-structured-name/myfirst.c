 ...
 * name and tighter ownership.
 *
 * Differences from Chapter 7 stage2-final:
 *   - The /dev node moves from /dev/myfirst0 to /dev/myfirst/0.
 *   - The node is group-accessible to the operator group (0660).
 *   - Everything else (softc, sysctls, exclusive open, unwind)
 *     is unchanged so the diff stays readable.
 */
...
myfirst_attach(device_t dev)
{
	...
	args.mda_gid = GID_OPERATOR;   /* was GID_WHEEL */
	args.mda_mode = 0660;          /* was 0600 */
	args.mda_si_drv1 = sc;

	/* Note the "myfirst/%d" form: this creates /dev/myfirst/0
	 * rather than /dev/myfirst0. Devfs creates the intermediate
	 * directory on demand.
	 */
	...
	device_printf(dev, "Attached; node at /dev/%s\n",
	    devtoname(sc->cdev));
	return (0);
