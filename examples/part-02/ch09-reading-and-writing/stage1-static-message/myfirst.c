 ...
 *
 * The driver carries a fixed kernel-side message and serves it to
 * readers through uiomove_frombuf(9).  Each open descriptor starts at
 * offset zero and reads its way through the message; a second read on
 * the same descriptor returns zero bytes (EOF).  Writes are still
 * discarded, the same /dev/null shape as Chapter 8 left us with.
 *
 * Differences from the Chapter 8 stage 2 source:
 *   - struct myfirst_softc gains a message pointer and its length.
 *   - struct myfirst_fh gains a per-descriptor read offset.
 *   - myfirst_read calls uiomove_frombuf(9) against the message.
 *   - myfirst_write stays a stub that consumes all bytes and returns
 *     zero.
 */
...
{
	device_set_desc(dev, "My First FreeBSD Driver (Chapter 9 Stage 1)");
...
myfirst_attach(device_t dev)
{
	...
	device_printf(dev,
	    "Attached; node at /dev/%s (alias /dev/myfirst), message=%zu bytes\n",
	    devtoname(sc->cdev), sc->message_len);
	return (0);
